...
void sample_g2o_2d(const std::string &filename, int max_iter, int min_iter, double robust_thre)
{
...
void sample_g2o_3d(const std::string &filename, int max_iter, int min_iter, double robust_thre)
{
    ...
    optimizer.setRobustThreshold(robust_thre);
    auto t0 = std::chrono::high_resolution_clock::now();
    p2o::Pose3DVec result = optimizer.optimizePath(nodes, con, max_iter, min_iter);
    auto t1 = std::chrono::high_resolution_clock::now();
    auto elapsed = std::chrono::duration_cast< std::chrono::microseconds> (t1-t0);
    std::cout << filename << ": " << elapsed.count()*1e-6 << "s" << std::endl;
    for(int i=1; i<result.size(); i++) {
        ofs << nodes[i].x << " " << nodes[i].y << " " << nodes[i].z << std::endl;
        ofs2 << result[i].x << " " << result[i].y << " " << result[i].z << std::endl;
    }
}

int main()
{
    sample_g2o_2d("intel.g2o", 20, 3, 1);
    sample_g2o_2d("manhattan3500.g2o", 20, 3, 1);

    // non-robust
    //sample_g2o("input_MITb_g2o.g2o", 100, 100, std::numeric_limits<double>::max());

    // robust
    sample_g2o_2d("mit_killian.g2o", 20, 20, 1);
